...
def test_dashboard_modal_firma_muestra_boton_de_firma_guardada_si_el_usuario_tiene_una(client, db):
    ana = _usuario_y_login(client, email="ana@test.es")
    ...
    resp = client.get("/")
    html = resp.data.decode()
    assert 'id="btn-firma-usar-guardada"' in html
...
def test_dashboard_caducada_muestra_botones_eliminar(client, db):
    ...
    assert "/publicaciones/eliminar-caducadas" in html
    assert f'/publicaciones/{pub.id}/eliminar' in html
    assert "Eliminar todos" in html
    assert "Eliminar" in html


def test_dashboard_activos_no_muestra_botones_eliminar_caducadas(client, db):
    """GET /?estado=activos NO debe mostrar los botones de borrado masivo de caducadas."""
    usuario = _usuario_y_login(client)
    franja = _franja(usuario.unidad.grupo_intercambio_id)
    _publicacion(usuario, franja, fecha_cedida=date(2026, 10, 1), fecha_aceptada=date(2026, 10, 2))
