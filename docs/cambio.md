...
- [ ] **Paso 0 — Preparación**
  - Confirmar que se trabaja directamente en `main` (excepción puntual a la norma de
    worktree por tarea, indicada explícitamente por el usuario para esta tarea).
  - Crear rama de trabajo local si se decide PR normal, o commitear directo en `main`
    según lo que pida el usuario en el momento de ejecutar cada paso.
