...
- [x] tests/test_servicio_planilla.py (27 → 20 tests) — Notas: une `test_añadir_turno_crea_registro` + `_crea_planilla_mes_borrador`; une `test_eliminar_turno_existente` + `_inexistente_devuelve_false`; parametriza `test_publicar_mes` (con/sin turnos previos); une los 3 tests de `test_tiene_mes_publicado_*` en uno; une los 2 de `test_franjas_trabajadas_en_fecha_*` en uno; une `test_establecer_estado_libre` + `test_estado_invalido_lanza_error`. Pendiente para una futura sesión: el grupo de `SalienteDia` (7 tests: crea_registro, es_idempotente, quitar_elimina, quitar_inexistente, coexiste_con_turno, limpiar_dia_elimina, get_salientes_mes) tiene más margen de fusión (crea_registro+es_idempotente, quitar_elimina+quitar_inexistente) — objetivo ~18 tests.
...
- [x] tests/test_planilla_relleno.py (19 → 15 tests) — Notas: elimina `test_rango_un_solo_dia`, duplicado exacto de `test_rango_un_solo_dia_sigue_funcionando` (que además comprueba el commit único); une `test_rango_aplica_estado_a_varios_dias` + `_turno_a_varios_dias` en un solo test; une `test_multiples_aplica_a_fechas_seleccionadas` + `_turno_a_fechas_seleccionadas`; une `test_vacios_aplicar_rellena_dias_sin_asignar` + `_con_turno_rellena_vacios` (con logout explícito entre los dos usuarios para evitar contaminación de `unidad_activa` en sesión). Se mantienen separados los tests de regresión de commits N+1 y el de rollback atómico por ser casos de negocio distintos.
- [x] tests/test_planilla_multi_unidad.py (15 → 6 tests) — Notas: cada grupo de tests que verificaba aislamiento por unidad de una misma función de servicio (turnos, estados, saliente, notas, publicar_mes) se une en un único test que encadena las aserciones sobre ambas unidades, en vez de un test por función y por unidad. Se mantiene aparte `test_planilla_mes_unico_por_usuario_y_unidad` por ser una regresión de integridad de BD (IntegrityError) no relacionada con el aislamiento.
