"""E2E: firma dibujada al confirmar un match directo simétrico genera y
firma su DocumentoCambio automáticamente.

Ejercita el mecanismo real de captura de la firma (canvas + eventos de
puntero → PNG en base64) en Chromium vía Playwright, algo que un test con
el cliente de Flask no puede comprobar porque no ejecuta JS.
"""
...
from app.models import DocumentoCambio, FranjaHoraria, MatchCambio, MatchParticipacion, PublicacionCambio, TurnoCedido, TurnoAceptado, Usuario
from app.services.registro import registrar_usuario


@pytest.fixture
def escenario_match_simetrico(e2e_app, usuario):
    with e2e_app.app_context():
        ...
        ta_ana = TurnoAceptado(publicacion_id=pub_ana.id, fecha=fecha_pedro, franja_horaria_id=franja.id)
        ...
        ta_pedro = TurnoAceptado(publicacion_id=pub_pedro.id, fecha=fecha_ana, franja_horaria_id=franja.id)
...
def test_confirmar_match_simetrico_dibujando_firma_crea_documento(
    pagina_autenticada, live_server, escenario_match_simetrico, e2e_app,
):
    page = pagina_autenticada
    match_id = escenario_match_simetrico
    ...
    with e2e_app.app_context():
        documento = DocumentoCambio.query.filter_by(match_id=match_id).first()
        assert documento is not None
        assert documento.estado == "pendiente_firmas"
        assert len(documento.firmas) == 1


def test_confirmar_sin_dibujar_firma_no_confirma(pagina_autenticada, live_server, escenario_match_simetrico):
    page = pagina_autenticada
    page.goto(f"{live_server}/")
